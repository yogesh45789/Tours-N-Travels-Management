...
public class RemoveCustomer extends JFrame {
	...
	public RemoveCustomer() throws SQLException {
		setBounds(340, 120, 850, 550);
		contentPane = new JPanel();
		setContentPane(contentPane);
		contentPane.setLayout(null);

		ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("Tours_n_Travel_Management_System/icons/delete.png"));
		Image i3 = i1.getImage().getScaledInstance(300, 300, Image.SCALE_DEFAULT);
		ImageIcon i2 = new ImageIcon(i3);
		JLabel l1 = new JLabel(i2);
		l1.setBounds(520, 120, 300, 300);
		add(l1);
		...
		JLabel lb3 = new JLabel("Id number :");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					c.s.executeUpdate("delete from customer where number = '" + s1 + "'");
					c.s.executeUpdate("delete from account where username = '" + s3 + "'");
					// c.s.executeUpdate( "delete from customer where username = '"+s1+"'");
					c.s.executeUpdate("delete from bookpackage where number = '" + s1 + "'");
					c.s.executeUpdate("delete from bookhotel where number = '" + s1 + "'");
