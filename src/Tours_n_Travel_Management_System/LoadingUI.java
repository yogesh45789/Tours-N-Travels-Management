...
public class LoadingUI extends JFrame implements Runnable {
        ...
        try {
            for (int i = 0; i < 200; i++) {
                ...
                if (v < m) {
                    ...
                    i = 201;
                    setVisible(false);
                    new bg().setVisible(false);
                    new TravelPortal(username).setVisible(true);
                }
                Thread.sleep(50);
            }
        } catch (Exception e) {
    ...
    public LoadingUI(String username) {
        ...
        JLabel lbllibraryManagement = new JLabel("Tours & Travel App");
